        ...
        <div id="tabbody-div">
            <form enctype="multipart/form-data" action="" method="post">
                <table width="90%" class="table" align="center">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            开始时间：<input type="text" name="start" value="<?php if(($info["start"]) != "0"): echo (date('Y-m-d H:i:s',$info["start"])); endif; ?>"size="20" />
                            结束时间：<input type="text" name="end" value="<?php if(($info["start"]) != "0"): echo (date('Y-m-d H:i:s',$info["end"])); endif; ?>" size="20" />
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <select name="cate_id">
                                ...
                                <?php if(is_array($cate)): $i = 0; $__LIST__ = $cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(($vo["id"]) == $info["cate_id"]): ?>selected="selected"<?php endif; ?>><?php echo (str_repeat('--',$vo["lev"])); echo ($vo["cname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <script id="editor" type="text/plain" name="goods_body" value="" style="width:1024px;height:500px;"><?php echo ($info["goods_body"]); ?></script>
                        ...
                        <td colspan="2" id="showAttr">
                            <table width="90%"  align="center" >
                                <?php if(is_array($attr)): $i = 0; $__LIST__ = $attr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if(is_array($v)): $i = 0; $__LIST__ = $v;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        <td class="label">
                                            <?php if(($vo["attr_type"]) == "2"): ?><a href="javascript:;" onclick="clonethis(this)"><?php if(($i) == "1"): ?>[+]<?php else: ?>[-]<?php endif; ?></a><?php endif; echo ($vo["attr_name"]); ?>：
                                        </td>
                                        <td>
                                            <?php if(($vo["attr_input_type"]) == "1"): ?><input type="text" name="attr[<?php echo ($vo["attr_id"]); ?>][]" value="<?php echo ($vo["attr_values"]); ?>"/>
                                                <?php else: ?>
                                                <select name="attr[<?php echo ($vo["attr_id"]); ?>][]" id="">
                                                    <?php if(is_array($vo["attr_value"])): $i = 0; $__LIST__ = $vo["attr_value"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option <?php if(($vo["attr_values"]) == $v): ?>selected="selected"<?php endif; ?>>
                                                            <?php echo ($v); ?>
                ...
                <table width="90%" class="table pic" align="center" style="display:none;">
                    <tr>
                        <td colspan="2">
                            <?php if(is_array($goods_img_list)): $i = 0; $__LIST__ = $goods_img_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div style="width: 100px; height:140px;float:left;margin:0 10px;" ><img src="/<?php echo ($vo["goods_thumb"]); ?>" width="100">
